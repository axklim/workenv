...
- **One dependency, deliberately.** `cmd/we` parses flags with
  `github.com/jessevdk/go-flags` (`golang.org/x/sys` comes with it);
  everything else is the standard library, the path template
  (`text/template`) included. A second one needs a reason that beats
  writing it.
